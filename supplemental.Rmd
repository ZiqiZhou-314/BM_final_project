...
```{r Residual plots for model 1994}
par(mfrow=c(2,2))
plot(f1)
```

```{r Looking at Salary in 1995}
...
```{r Residual plots for 1995}
par(mfrow=c(2,2))
plot(f2)
```

```{r Average Salary of 1994 and 1995}
mean.salary=(Sal94+Sal95)/2
hist(mean.salary)
log.mean=log(mean.salary)
hist(log.mean)
...
plot(f4)
```
Supplemental figure 2 Cook’s distance, quantile-quantile plot, and residuals plots show that the 184th observation is an influential point
