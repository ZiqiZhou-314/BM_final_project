---
title: "Supplemental"
author: "Group 20 - James Ng, Jennifer Tang, Qimin Zhang, Ziqi Zhou"
date: "12/14/2019"
...
```{r distribution of salary, echo=FALSE, message=FALSE, warning=FALSE}
par(mfrow=c(2,3))
hist(Sal94)
hist(Sal95)
...
hist(log.mean)
```
**Supplemental Figure 1 -** Distribution of salary for 1994(a), 1995(b), and the average of both academic years(c) is right skewed. Log Transformation of 1994(d), 1995(e), and averaged salary(f) shows a normal distribution.

``` {r Looking at Salary in 1994, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r Residual plots for model 1994, echo=FALSE, message=FALSE, warning=FALSE}
# par(mfrow=c(2,2))
# plot(f1)
```

```{r Looking at Salary in 1995, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r Residual plots for 1995, echo=FALSE, message=FALSE, warning=FALSE}
# par(mfrow=c(2,2))
# plot(f2)
```

```{r Average Salary of 1994 and 1995, echo=FALSE, message=FALSE, warning=FALSE}
mean.salary=(Sal94+Sal95)/2
# hist(mean.salary)
log.mean=log(mean.salary)
# hist(log.mean)
...
plot(f4)
```
**Supplemental figure 2 -** Residuals vs. Fitted (a), quantile-quantile plot(b), scale location plot(c) and Cook’s distance plot(d) all show the 184th observation is an outlier and potential influential point

## Supplemental Table 1
```{r, echo=FALSE, message=FALSE, warning=FALSE}
knitr::kable(summary(tab1))
#summary(tab1, text=TRUE)
```

**Supplement Table 1 -** General descriptive statistics about the data set.
...
**Supplement Table 2 -** Final model of the data set. Coefficient estimates, standard errors and p values are included.


## Supplemental Table 3
```{r, echo=FALSE, message=FALSE, warning=FALSE}
knitr::kable(anova(f4))
```

**Supplement Table 3 -** Anova test of the model. The p-value for gender was 0.186. This value means that gender as the main covariate of interest is not a significant contributor to the model when adjusting for department, primary emphasis, certifcation, experience, and rank.


```{r outliers and influence points, echo=FALSE, message=FALSE, warning=FALSE}
# rstandard(f4)
```
